...
- `--timeout=DURATION`: 指定時間後に自動終了（例: `5s`, `500ms`, `2m`）
...
son-etは1280x720ピクセルの仮想デスクトップを作成します。レガシーなFILLYスクリプト（通常640x480）は、この仮想デスクトップ内にウィンドウとして表示されます。
...
MIDIファイルを再生するには、楽器音源を含むSoundFont（.sf2）ファイルが必要です。

**配置場所:**
*   実行ファイルと同じディレクトリに配置
*   または `-sf <path>` オプションで指定
...
*   SoundFont（.sf2）ファイルが配置されているか確認
*   `-sf <path>` オプションでSoundFontのパスを指定してみてください
...
FILLYの作者である内田友幸さん、FILLYタイトルの作者の皆様、またFILLYを盛り上げたFFILLYのコミュニティの皆様方に感謝申し上げます。
