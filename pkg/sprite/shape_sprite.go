...
// 要件 8.1: 線を描画したスプライトを作成できる
// 要件 8.2: 矩形を描画したスプライトを作成できる
// 要件 8.3: 塗りつぶし矩形を描画したスプライトを作成できる
...
// 要件 8.1: 線を描画したスプライトを作成できる
// 親スプライトが指定された場合、図形は親の子として追加される（要件8対応）
func (ssm *ShapeSpriteManager) CreateLineSprite(
	...
	// スプライトを作成
	sprite := ssm.spriteManager.CreateSprite(img, parent)
...
func (ssm *ShapeSpriteManager) CreateFillCircleSprite(
	...
	vector.FillCircle(img, localCX, localCY, float32(radius), fillColor, false)
...
func (ssm *ShapeSpriteManager) RemoveShapeSprite(ss *ShapeSprite) {
	...
	// スプライトを削除
	if ss.sprite != nil {
		ssm.spriteManager.DeleteSprite(ss.sprite.ID())
	}
