...
func (m *mockGraphicsSystem) GetWindowCount() int {
...
func TestVMBuiltinMsgBox(t *testing.T) {
	...
	t.Run("MsgBox with message only", func(t *testing.T) {
		...
		result, err := fn(vm, []any{"Test message"})

		if err != nil {
			t.Errorf("expected no error, got %v", err)
		}
		// Should return IDOK
		if result != int64(IDOK) {
			t.Errorf("expected result %d (IDOK), got %v", IDOK, result)
		}
	})

	t.Run("MsgBox with message and warning icon", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		fn := vm.builtins["MsgBox"]
		// MB_ICONWARNING
		result, err := fn(vm, []any{"Warning message", int64(MB_ICONWARNING)})
	...
	t.Run("MsgBox with Yes/No buttons", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		fn := vm.builtins["MsgBox"]
		// MB_YESNO | MB_ICONQUESTION
		result, err := fn(vm, []any{"Continue?", int64(MB_YESNO | MB_ICONQUESTION)})

		if err != nil {
			t.Errorf("expected no error, got %v", err)
		}
		// In headless mode, returns IDYES (default for Yes/No)
		if result != int64(IDYES) {
			t.Errorf("expected result %d (IDYES), got %v", IDYES, result)
		}
	})

	t.Run("MsgBox with OK/Cancel buttons", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		fn := vm.builtins["MsgBox"]
		// MB_OKCANCEL | MB_ICONQUESTION
		result, err := fn(vm, []any{"Continue?", int64(MB_OKCANCEL | MB_ICONQUESTION)})

		if err != nil {
			t.Errorf("expected no error, got %v", err)
		}
		// In headless mode, returns IDOK (default for OK/Cancel)
		if result != int64(IDOK) {
			t.Errorf("expected result %d (IDOK), got %v", IDOK, result)
		}
	})

	t.Run("MsgBox with no arguments", func(t *testing.T) {
		vm := New([]opcode.OpCode{})
		...
		// Should return IDOK even with no arguments
		if result != int64(IDOK) {
			t.Errorf("expected result %d (IDOK), got %v", IDOK, result)
		}
	})

	t.Run("MsgBox with various button types", func(t *testing.T) {
		vm := New([]opcode.OpCode{})
		fn := vm.builtins["MsgBox"]
		...
		testCases := []struct {
			...
			{"OK button", MB_OK, IDOK},
			{"OK/Cancel buttons", MB_OKCANCEL, IDOK},
			{"Abort/Retry/Ignore buttons", MB_ABORTRETRYIGNORE, IDABORT},
			{"Yes/No buttons", MB_YESNO, IDYES},
			{"Retry/Cancel buttons", MB_RETRYCANCEL, IDRETRY},
			{"Cancel/Try Again/Continue buttons", MB_CANCELTRYCONTINUE, IDCANCEL},
		}

		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				result, err := fn(vm, []any{"Test", tc.flags})
				if err != nil {
					t.Errorf("expected no error, got %v", err)
				}
				if result != int64(tc.expectedReturn) {
					t.Errorf("expected result %d, got %v", tc.expectedReturn, result)
				}
			})
		}
	})

	t.Run("MsgBox with various icon types", func(t *testing.T) {
		vm := New([]opcode.OpCode{})
		fn := vm.builtins["MsgBox"]
		...
		testCases := []struct {
			...
			{"error icon", MB_ICONERROR},
			{"question icon", MB_ICONQUESTION},
			{"warning icon", MB_ICONWARNING},
...
// TestVMBuiltinGetSysTime tests the GetSysTime built-in function.
func TestVMBuiltinGetSysTime(t *testing.T) {
	t.Run("GetSysTime is registered as built-in", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		// Verify GetSysTime is registered
		if _, ok := vm.builtins["GetSysTime"]; !ok {
			t.Error("expected GetSysTime to be registered as built-in function")
		}
	})

	t.Run("GetSysTime returns current time in seconds", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		fn := vm.builtins["GetSysTime"]

		// Get time before call
		...
		// Result should be an int64
		timeValue, ok := result.(int64)
		if !ok {
			t.Errorf("expected int64 result, got %T", result)
		}

		// Time should be between before and after (or equal)
		if timeValue < before || timeValue > after {
			t.Errorf("expected time between %d and %d, got %d", before, after, timeValue)
		}
	})

	t.Run("GetSysTime returns increasing values", func(t *testing.T) {
		vm := New([]opcode.OpCode{})
		fn := vm.builtins["GetSysTime"]

		// Get first time
		result1, err1 := fn(vm, []any{})
		if err1 != nil {
			t.Errorf("expected no error, got %v", err1)
		}
		time1, _ := result1.(int64)

		// Wait for at least 1 second
		time.Sleep(1100 * time.Millisecond)

		// Get second time
		result2, err2 := fn(vm, []any{})
		if err2 != nil {
			t.Errorf("expected no error, got %v", err2)
		}
		time2, _ := result2.(int64)

		// Second time should be at least 1 second greater than first
		if time2 <= time1 {
			t.Errorf("expected time2 (%d) > time1 (%d)", time2, time1)
		}

		// Difference should be approximately 1 second
		diff := time2 - time1
		if diff < 1 || diff > 2 {
			t.Errorf("expected difference of 1-2 seconds, got %d", diff)
		}
	})

	t.Run("GetSysTime can be used for timing", func(t *testing.T) {
		vm := New([]opcode.OpCode{})
		fn := vm.builtins["GetSysTime"]

		// Start time
		start, _ := fn(vm, []any{})
		startTime := start.(int64)

		// Do some work (sleep for known duration)
		...
		// Elapsed should be approximately the sleep duration (in seconds)
		expectedMin := int64(1) // At least 1 second
		expectedMax := int64(3) // At most 3 seconds (with tolerance)

		if elapsed < expectedMin || elapsed > expectedMax {
			t.Errorf("expected elapsed time between %d and %d seconds, got %d seconds", expectedMin, expectedMax, elapsed)
		}
	})
}

// TestContainsOpSetStep tests the containsOpSetStep function.
// Requirement 1.1: When an event handler is registered, THE System SHALL scan the handler's OpCodes for OpSetStep
func TestContainsOpSetStep(t *testing.T) {
	t.Run("returns true when OpSetStep is present at top level", func(t *testing.T) {
		opcodes := []opcode.OpCode{
		...
		if !result {
			t.Error("expected containsOpSetStep to return true when OpSetStep is present")
		}
	})

	t.Run("returns false when OpSetStep is not present", func(t *testing.T) {
		opcodes := []opcode.OpCode{
			{Cmd: opcode.Call, Args: []any{"func1"}},
		...
		result := containsOpSetStep(opcodes)
		if result {
			t.Error("expected containsOpSetStep to return false when OpSetStep is not present")
		}
	})

	t.Run("returns true when OpSetStep is nested in If block", func(t *testing.T) {
		// Simulates: if (condition) { step(10) { ... } }
		nestedOpcodes := []opcode.OpCode{
			{Cmd: opcode.SetStep, Args: []any{int64(10)}},
			{Cmd: opcode.Call, Args: []any{"nestedFunc"}},
		}
		opcodes := []opcode.OpCode{
			{Cmd: opcode.If, Args: []any{
				opcode.OpCode{Cmd: opcode.BinaryOp, Args: []any{"==", opcode.Variable("x"), int64(1)}},
				nestedOpcodes, // then block
				nil,           // else block
			}},
		}
		...
		result := containsOpSetStep(opcodes)
		if !result {
			t.Error("expected containsOpSetStep to return true when OpSetStep is nested in If block")
		}
	})

	t.Run("returns true when OpSetStep is nested in For block", func(t *testing.T) {
		// Simulates: for (i = 0; i < 10; i++) { step(5) { ... } }
		bodyOpcodes := []opcode.OpCode{
			{Cmd: opcode.SetStep, Args: []any{int64(5)}},
			{Cmd: opcode.Call, Args: []any{"loopFunc"}},
		}
		opcodes := []opcode.OpCode{
			{Cmd: opcode.For, Args: []any{
				opcode.OpCode{Cmd: opcode.Assign, Args: []any{opcode.Variable("i"), int64(0)}},                                                                              // init
				opcode.OpCode{Cmd: opcode.BinaryOp, Args: []any{"<", opcode.Variable("i"), int64(10)}},                                                                      // condition
				opcode.OpCode{Cmd: opcode.Assign, Args: []any{opcode.Variable("i"), opcode.OpCode{Cmd: opcode.BinaryOp, Args: []any{"+", opcode.Variable("i"), int64(1)}}}}, // update
				bodyOpcodes, // body
			}},
		}
		...
		result := containsOpSetStep(opcodes)
		if !result {
			t.Error("expected containsOpSetStep to return true when OpSetStep is nested in For block")
		}
	})

	t.Run("returns true when OpSetStep is deeply nested", func(t *testing.T) {
		// Simulates: if (a) { if (b) { step(10) { ... } } }
		deepNestedOpcodes := []opcode.OpCode{
			{Cmd: opcode.SetStep, Args: []any{int64(10)}},
		}
		innerIfOpcodes := []opcode.OpCode{
			{Cmd: opcode.If, Args: []any{
				opcode.OpCode{Cmd: opcode.BinaryOp, Args: []any{"==", opcode.Variable("b"), int64(1)}},
				deepNestedOpcodes,
				nil,
			}},
		}
		opcodes := []opcode.OpCode{
			{Cmd: opcode.If, Args: []any{
				opcode.OpCode{Cmd: opcode.BinaryOp, Args: []any{"==", opcode.Variable("a"), int64(1)}},
				innerIfOpcodes,
				nil,
			}},
		}
		...
		result := containsOpSetStep(opcodes)
		if !result {
			t.Error("expected containsOpSetStep to return true when OpSetStep is deeply nested")
		}
	})

	t.Run("returns false for empty opcode sequence", func(t *testing.T) {
		opcodes := []opcode.OpCode{}

		result := containsOpSetStep(opcodes)
		if result {
			t.Error("expected containsOpSetStep to return false for empty opcode sequence")
		}
	})

	t.Run("returns false for nil opcode sequence", func(t *testing.T) {
		var opcodes []opcode.OpCode = nil

		result := containsOpSetStep(opcodes)
		if result {
			t.Error("expected containsOpSetStep to return false for nil opcode sequence")
		}
	})

	t.Run("returns true when OpSetStep is first opcode", func(t *testing.T) {
		opcodes := []opcode.OpCode{
			{Cmd: opcode.SetStep, Args: []any{int64(10)}},
			{Cmd: opcode.Call, Args: []any{"func1"}},
		}
		...
		result := containsOpSetStep(opcodes)
		if !result {
			t.Error("expected containsOpSetStep to return true when OpSetStep is first opcode")
		}
	})

	t.Run("returns true when OpSetStep is last opcode", func(t *testing.T) {
		opcodes := []opcode.OpCode{
			{Cmd: opcode.Call, Args: []any{"func1"}},
			{Cmd: opcode.SetStep, Args: []any{int64(10)}},
		}
		...
		result := containsOpSetStep(opcodes)
		if !result {
			t.Error("expected containsOpSetStep to return true when OpSetStep is last opcode")
		}
	})

	t.Run("returns false when nested blocks have no OpSetStep", func(t *testing.T) {
		// Simulates: if (condition) { call(); }
		nestedOpcodes := []opcode.OpCode{
			{Cmd: opcode.Call, Args: []any{"nestedFunc"}},
		}
		opcodes := []opcode.OpCode{
		...
		result := containsOpSetStep(opcodes)
		if result {
			t.Error("expected containsOpSetStep to return false when nested blocks have no OpSetStep")
		}
	})
}

// TestExecuteRegisterEventHandlerHasStepBlock tests that executeRegisterEventHandler correctly sets HasStepBlock.
...
func TestExecuteRegisterEventHandlerHasStepBlock(t *testing.T) {
	t.Run("sets HasStepBlock=true when handler contains step()", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		// Create an OpRegisterEventHandler with step() block
		// Simulates: mes(TIME) { step(10) { ... } }
		registerOp := opcode.OpCode{
			Cmd: opcode.RegisterEventHandler,
			Args: []any{
				"TIME",
				[]opcode.OpCode{
					{Cmd: opcode.SetStep, Args: []any{int64(10)}},
					{Cmd: opcode.Call, Args: []any{"someFunc", []any{}}},
				},
			},
		}

		// Execute the registration
		result, err := vm.Execute(registerOp)
		if err != nil {
			t.Fatalf("expected no error, got %v", err)
		}

		// Get the handler ID from the result
		handlerID, ok := result.(string)
		if !ok {
			t.Fatalf("expected handler ID string, got %T", result)
		}

		// Get the registered handler
		handler, exists := vm.handlerRegistry.GetHandler(handlerID)
		if !exists {
		...
		if !handler.HasStepBlock {
			t.Error("expected HasStepBlock to be true for handler with step() block")
		}
	})

	t.Run("sets HasStepBlock=false when handler does not contain step()", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		// Create an OpRegisterEventHandler without step() block
		// Simulates: mes(LBDOWN) { call(); }
		registerOp := opcode.OpCode{
			Cmd: opcode.RegisterEventHandler,
			Args: []any{
				"LBDOWN",
				[]opcode.OpCode{
					{Cmd: opcode.Call, Args: []any{"onClick", []any{}}},
					{Cmd: opcode.Assign, Args: []any{opcode.Variable("x"), int64(1)}},
				},
			},
		}
		...
		if handler.HasStepBlock {
			t.Error("expected HasStepBlock to be false for handler without step() block")
		}
	})

	t.Run("sets HasStepBlock=true when step() is nested in If block", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		// Create an OpRegisterEventHandler with nested step() in If block
		// Simulates: mes(TIME) { if (condition) { step(5) { ... } } }
		nestedOpcodes := []opcode.OpCode{
			{Cmd: opcode.SetStep, Args: []any{int64(5)}},
			{Cmd: opcode.Call, Args: []any{"nestedFunc", []any{}}},
		}
		registerOp := opcode.OpCode{
			Cmd: opcode.RegisterEventHandler,
			Args: []any{
		...
		// Verify HasStepBlock is true (nested step() detected)
		if !handler.HasStepBlock {
			t.Error("expected HasStepBlock to be true for handler with nested step() block")
		}
	})

	t.Run("sets HasStepBlock=true when step() is nested in For block", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		// Create an OpRegisterEventHandler with nested step() in For block
		// Simulates: mes(MIDI_TIME) { for (i=0; i<10; i++) { step(1) { ... } } }
		forBodyOpcodes := []opcode.OpCode{
			...
			Args: []any{
				...
				[]opcode.OpCode{
					{Cmd: opcode.For, Args: []any{
						...
						opcode.OpCode{Cmd: opcode.Assign, Args: []any{opcode.Variable("i"), opcode.OpCode{Cmd: opcode.BinaryOp, Args: []any{"+", opcode.Variable("i"), int64(1)}}}},
						forBodyOpcodes,
					}},
				},
			},
		...
		// Verify HasStepBlock is true (nested step() detected)
		if !handler.HasStepBlock {
			t.Error("expected HasStepBlock to be true for handler with step() nested in For block")
		}
	})

	t.Run("sets HasStepBlock=false for empty handler body", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		// Create an OpRegisterEventHandler with empty body
		// Simulates: mes(CLICK) { }
		registerOp := opcode.OpCode{
			Cmd: opcode.RegisterEventHandler,
			Args: []any{
		...
		// Verify HasStepBlock is false
		if handler.HasStepBlock {
			t.Error("expected HasStepBlock to be false for empty handler body")
		}
	})

	t.Run("sets HasStepBlock=true when step() is deeply nested", func(t *testing.T) {
		vm := New([]opcode.OpCode{})

		// Create an OpRegisterEventHandler with deeply nested step()
		// Simulates: mes(TIME) { if (a) { if (b) { step(1) { ... } } } }
		deeplyNestedOpcodes := []opcode.OpCode{
			{Cmd: opcode.SetStep, Args: []any{int64(1)}},
		}
		innerIfOpcodes := []opcode.OpCode{
			{Cmd: opcode.If, Args: []any{
		...
		// Verify HasStepBlock is true (deeply nested step() detected)
		if !handler.HasStepBlock {
			t.Error("expected HasStepBlock to be true for handler with deeply nested step() block")
		}
	})
}
