...
type EventHandler struct {
	...
	// HasStepBlock indicates whether this handler was created with a step() block.
	// Handlers with step() blocks are removed after completion.
	// Handlers without step() blocks (e.g., click handlers) reset their PC and continue.
	HasStepBlock bool
...
func (eh *EventHandler) Execute(event *Event) error {
	...
	for eh.CurrentPC < len(eh.OpCodes) {
		...
		if err != nil {
			...
			if errors.As(err, &runtimeErr) && runtimeErr.IsFatal() {
				eh.VM.log.Error("Fatal error in handler, stopping execution", "handler", eh.ID, "error", err)
				// Restore the previous handler and local scope before returning
	...
	// Handler completed all OpCodes
	//
	// step()ブロックを持つハンドラ（HasStepBlock == true）:
	// - step内の全処理が完了したら、ハンドラを削除
	// - 次のイベントが来ても、このハンドラは呼ばれない
	//
	// step()ブロックを持たないハンドラ（HasStepBlock == false）:
	// - 例: mes(LBDOWN){...} のようなクリックハンドラ
	// - 処理完了後、CurrentPCをリセットして次のイベントに備える
	// - イベントが来るたびに最初から実行される
	if eh.HasStepBlock {
		// step()ブロック完了 → ハンドラを削除
		eh.VM.log.Debug("Handler with step() completed, removing handler", "handler", eh.ID)
		eh.Remove()
	} else {
		// step()無しのハンドラ → 次のイベントに備えてリセット
		eh.CurrentPC = 0
		eh.VM.log.Debug("Handler without step() completed, resetting PC", "handler", eh.ID)
	}
...
// GetHandlerByNumber returns a handler by its numeric ID.
// The numeric ID corresponds to the registration order (1-based).
// Returns the handler and true if found, nil and false otherwise.
func (hr *HandlerRegistry) GetHandlerByNumber(num int) (*EventHandler, bool) {
	hr.mu.RLock()
	defer hr.mu.RUnlock()

	id := generateHandlerID(num)
	handler, ok := hr.handlersByID[id]
	return handler, ok
}
	...
	for _, handler := range handlers {
		if handler.Active {
			if err := handler.Execute(event); err != nil {
				// Check if this is a fatal error - propagate it up (use errors.As to unwrap wrapped errors)
				var runtimeErr *RuntimeError
				if errors.As(err, &runtimeErr) && runtimeErr.IsFatal() {
					return err
				}
