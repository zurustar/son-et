...
func (vm *VM) evaluateValue(value any) (any, error) {
	...
	case opcode.Variable:
		...
		if !ok {
			// Requirement 11.5: When variable is not found, system creates it with default value 0.
			// This ensures subsequent accesses don't trigger warnings.
			vm.log.Debug("Variable not found, creating with default value 0", "name", string(v))
			vm.GetCurrentScope().Set(string(v), int64(0))
			return int64(0), nil
		}
		return resolved, nil
...
func (vm *VM) executeAssign(op opcode.OpCode) (any, error) {
	...
	// Convert []any{} to Array type for array initialization
	if slice, ok := value.([]any); ok {
		value = NewArrayFromSlice(slice)
	}
...
func (vm *VM) executeArrayAssign(op opcode.OpCode) (any, error) {
	...
	// Requirement 19.4: When array index is negative, system logs warning and returns zero.
	if index < 0 {
		vm.log.Warn("Negative array index, returning 0", "array", string(arrayName), "index", index)
		return int64(0), nil
	}
...
func (vm *VM) callUserFunction(fn *FunctionDef, args []any) (any, error) {
	...
	if fn.Name == "main" {
		...
		// FILLY uses dynamic scoping: functions can access variables from the calling scope.
		// This is important for event handlers that call functions with variables
		// declared in the enclosing scope (e.g., start() function's local variables
		// accessed by functions called from mes() handlers).
		localScope = NewScope(vm.GetCurrentScope())
	}
	...
	for _, op := range fn.Body {
		...
		if err != nil {
			// Check if this is a fatal error using errors.As to properly unwrap
			// Requirement 20.8: When stack overflow occurs, system logs error and terminates execution.
			// Requirement 11.6: When undefined function is called, system terminates execution.
			var runtimeErr *RuntimeError
			if errors.As(err, &runtimeErr) && runtimeErr.IsFatal() {
				vm.log.Error("Fatal error in function", "function", fn.Name, "error", err, "type", runtimeErr.Type)
...
// Requirement 11.4: When array index is out of range, system auto-expands array and returns zero.
func (vm *VM) executeArrayAccess(op opcode.OpCode) (any, error) {
	...
	// Get array variable name (if available) for auto-expansion
	var arrayVarName opcode.Variable
	hasVarName := false
	if varName, ok := op.Args[0].(opcode.Variable); ok {
	...
	// Requirement 19.4: When array index is negative, system logs warning and returns zero.
	if index < 0 {
		vm.log.Warn("Negative array index, returning 0", "array", string(arrayVarName), "index", index)
		return int64(0), nil
	}
	...
	if arr, ok := arrayVal.(*Array); ok {
		...
		if !found {
			// Auto-expand array and set default value 0
			// This ensures subsequent accesses don't trigger expansion
			vm.log.Debug("Array auto-expanding on read access", "array", string(arrayVarName), "index", index, "oldLength", arr.Len())
			arr.Set(index, int64(0))
			return int64(0), nil
		}
		return val, nil
	}

	// If arrayVal is not an Array type but we have a variable name,
	// create a new Array and store it in scope
	if hasVarName {
		// Create new array with the required size
		newArr := NewArray(int(index) + 1)
		vm.GetCurrentScope().Set(string(arrayVarName), newArr)
		vm.log.Debug("Array created on read access", "array", string(arrayVarName), "index", index)
		return int64(0), nil
	}

	// Handle legacy []any slice (shouldn't happen often)
	arr, ok := arrayVal.([]any)
	if !ok {
		...
		vm.log.Debug("Non-array value accessed with index > 0, returning 0", "index", index)
		return int64(0), nil
	}

	if int(index) >= len(arr) {
		vm.log.Debug("Legacy slice index out of range, returning 0", "index", index, "length", len(arr))
		return int64(0), nil
	}
