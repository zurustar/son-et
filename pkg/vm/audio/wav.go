...
// SetFileSystem sets the file system interface for reading WAV files.
// This allows the WAVPlayer to read files from embedded file systems.
//
// Parameters:
//   - fs: The FileSystem interface to use for file access
func (wp *WAVPlayer) SetFileSystem(fs fileutil.FileSystem) {
	wp.mu.Lock()
	defer wp.mu.Unlock()
	wp.fs = fs
}
...
func (wp *WAVPlayer) GetFileSystem() fileutil.FileSystem {
	wp.mu.Lock()
	defer wp.mu.Unlock()
	return wp.fs
}
