...
// TestArrayClear tests the Clear method.
// Requirement 5.1: DelArrayAll が配列引数で呼び出された場合、全要素を削除し、配列サイズを0にする
// Requirement 5.2: DelArrayAll が空の配列で呼び出された場合、エラーを発生させずに正常に完了する
func TestArrayClear(t *testing.T) {
	t.Run("clears non-empty array to size 0", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{int64(10), int64(20), int64(30)})
		if arr.Len() != 3 {
			t.Fatalf("expected initial length 3, got %d", arr.Len())
		}

		arr.Clear()

		if arr.Len() != 0 {
			t.Errorf("expected length 0 after Clear, got %d", arr.Len())
		}
	})

	t.Run("clears empty array without error", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{})

		// Should not panic or error
		arr.Clear()

		if arr.Len() != 0 {
			t.Errorf("expected length 0 after Clear on empty array, got %d", arr.Len())
		}
	})

	t.Run("array is reusable after Clear", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{int64(1), int64(2), int64(3)})
		arr.Clear()

		// After Clear, setting elements should work normally
		arr.Set(0, int64(42))
		if arr.Len() != 1 {
			t.Errorf("expected length 1 after Set on cleared array, got %d", arr.Len())
		}
		elem, ok := arr.Get(0)
		if !ok {
			t.Fatal("expected Get(0) to succeed on cleared+set array")
		}
		if elem != int64(42) {
...
// TestArrayDeleteAt tests the DeleteAt method.
// Requirement 6.1: DelArrayAt が配列とインデックスで呼び出された場合、指定位置の要素を削除し、後続の要素を前方にシフトし、配列サイズを1減少させる
// Requirement 6.2: DelArrayAt のインデックスが配列の範囲外（負数または配列サイズ以上）の場合、エラーを返す
// Requirement 6.3: DelArrayAt で配列の最後の要素を削除した場合、配列サイズを1減少させ、シフト操作を行わない
func TestArrayDeleteAt(t *testing.T) {
	t.Run("deletes element at valid index and shifts", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{int64(10), int64(20), int64(30), int64(40), int64(50)})
		...
		if arr.Len() != 4 {
			t.Errorf("expected length 4 after DeleteAt, got %d", arr.Len())
		}

		// Elements before deleted index should be unchanged
		elem0, _ := arr.Get(0)
		elem1, _ := arr.Get(1)
		if elem0 != int64(10) {
			t.Errorf("expected arr[0] = 10, got %v", elem0)
		...
		// Elements after deleted index should shift forward
		elem2, _ := arr.Get(2)
		elem3, _ := arr.Get(3)
		if elem2 != int64(40) {
			t.Errorf("expected arr[2] = 40 (shifted), got %v", elem2)
		}
		if elem3 != int64(50) {
			t.Errorf("expected arr[3] = 50 (shifted), got %v", elem3)
		}
	})
	...
	t.Run("deletes first element", func(t *testing.T) {
		...
		elem1, _ := arr.Get(1)
		if elem0 != int64(20) {
			t.Errorf("expected arr[0] = 20, got %v", elem0)
		}
		if elem1 != int64(30) {
			t.Errorf("expected arr[1] = 30, got %v", elem1)
		}
	})

	t.Run("deletes last element without shift", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{int64(10), int64(20), int64(30)})
		...
		// Existing elements should be unchanged
		elem0, _ := arr.Get(0)
		elem1, _ := arr.Get(1)
		if elem0 != int64(10) {
			t.Errorf("expected arr[0] = 10, got %v", elem0)
	...
	t.Run("returns error for index equal to size", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{int64(10), int64(20), int64(30)})
	...
	t.Run("returns error for index greater than size", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{int64(10), int64(20), int64(30)})

		err := arr.DeleteAt(100)
		if err == nil {
			t.Fatal("expected error for index > size, got nil")
		}
	})
...
// TestArrayInsertAt tests the InsertAt method.
// Requirement 7.1: InsArrayAt が配列、インデックス、値で呼び出された場合、指定位置に値を挿入し、既存の要素を後方にシフトし、配列サイズを1増加させる
// Requirement 7.2: InsArrayAt のインデックスが配列の範囲外（負数または配列サイズ超過）の場合、エラーを返す
// Requirement 7.3: InsArrayAt で配列の先頭（インデックス0）に挿入した場合、全既存要素を後方にシフトし、先頭に新しい値を配置する
// Requirement 7.4: InsArrayAt で配列の末尾（インデックス=配列サイズ）に挿入した場合、配列末尾に値を追加し、配列サイズを1増加させる
func TestArrayInsertAt(t *testing.T) {
	t.Run("inserts at valid index and shifts elements", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{int64(10), int64(20), int64(30)})

		err := arr.InsertAt(1, int64(15))
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}

		if arr.Len() != 4 {
			t.Errorf("expected length 4 after InsertAt, got %d", arr.Len())
		}

		// Element before insertion point should be unchanged
		elem0, _ := arr.Get(0)
		if elem0 != int64(10) {
			t.Errorf("expected arr[0] = 10, got %v", elem0)
		}
		...
		elem1, _ := arr.Get(1)
		if elem1 != int64(15) {
			t.Errorf("expected arr[1] = 15 (inserted), got %v", elem1)
		}

		// Elements after insertion point should shift backward
		elem2, _ := arr.Get(2)
		elem3, _ := arr.Get(3)
		if elem2 != int64(20) {
			t.Errorf("expected arr[2] = 20 (shifted), got %v", elem2)
		}
		if elem3 != int64(30) {
			t.Errorf("expected arr[3] = 30 (shifted), got %v", elem3)
		}
	})

	t.Run("inserts at head shifts all elements", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{int64(10), int64(20), int64(30)})

		err := arr.InsertAt(0, int64(5))
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		...
		elem3, _ := arr.Get(3)
		if elem0 != int64(5) {
			t.Errorf("expected arr[0] = 5 (inserted), got %v", elem0)
		}
		if elem1 != int64(10) {
			t.Errorf("expected arr[1] = 10 (shifted), got %v", elem1)
		}
		if elem2 != int64(20) {
			t.Errorf("expected arr[2] = 20 (shifted), got %v", elem2)
		}
	...
	t.Run("inserts at tail (index == size)", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{int64(10), int64(20), int64(30)})

		err := arr.InsertAt(3, int64(40))
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		...
		elem3, _ := arr.Get(3)
		if elem3 != int64(40) {
			t.Errorf("expected arr[3] = 40 (appended), got %v", elem3)
		}
	})
	...
	t.Run("returns error for index greater than size", func(t *testing.T) {
		...
		err := arr.InsertAt(4, int64(99))
		if err == nil {
			t.Fatal("expected error for index > size, got nil")
		}
	})

	t.Run("inserts into empty array at index 0", func(t *testing.T) {
		arr := NewArrayFromSlice([]any{})

		err := arr.InsertAt(0, int64(42))
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
