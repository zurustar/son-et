...
func TestStackOverflowDetection(t *testing.T) {
	t.Run("detects stack overflow in recursive function", func(t *testing.T) {
		...
		// Check that it's a RuntimeError with STACK_OVERFLOW type
		var runtimeErr *RuntimeError
		if !errors.As(err, &runtimeErr) {
			t.Fatalf("expected RuntimeError, got %T: %v", err, err)
		}
		if runtimeErr.Type != ErrorStackOverflow {
			t.Errorf("expected error type STACK_OVERFLOW, got %s", runtimeErr.Type)
		}
		if !strings.Contains(err.Error(), "stack overflow") {
			t.Errorf("expected error message to contain 'stack overflow', got: %v", err)
		}
	})
}
...
// TestUndefinedFunctionError tests that undefined function calls return fatal errors.
func TestUndefinedFunctionError(t *testing.T) {
	t.Run("undefined function returns error", func(t *testing.T) {
		vm := New([]opcode.OpCode{})
		opcode := opcode.OpCode{
			Cmd:  opcode.Call,
			Args: []any{"UndefinedFunction"},
		}
		...
		// Check that it's a RuntimeError with UNDEFINED_FUNCTION type
		var runtimeErr *RuntimeError
		if !errors.As(err, &runtimeErr) {
			t.Fatalf("expected RuntimeError, got %T", err)
		}
		if runtimeErr.Type != ErrorUndefinedFunc {
			t.Errorf("expected error type UNDEFINED_FUNCTION, got %s", runtimeErr.Type)
		}
		if !runtimeErr.IsFatal() {
			t.Error("expected undefined function error to be fatal")
		}
	})

	t.Run("fatal error propagates from user function body", func(t *testing.T) {
		vm := New([]opcode.OpCode{})
		// Register a user-defined function that calls an undefined function
		vm.functions["testFunc"] = &FunctionDef{
			Name:       "testFunc",
			Parameters: []FunctionParam{},
			Body: []opcode.OpCode{
				{
		...
		_, err := vm.executeCall(opcode)
		if err == nil {
			t.Fatal("expected error to propagate from user function, got nil")
		}

		// Check that the fatal error propagated
		var runtimeErr *RuntimeError
		if !errors.As(err, &runtimeErr) {
			t.Fatalf("expected RuntimeError, got %T", err)
		}
		if !runtimeErr.IsFatal() {
			t.Error("expected fatal error to propagate")
		}
	})

	t.Run("nested function call propagates fatal error", func(t *testing.T) {
		vm := New([]opcode.OpCode{})
		// Register nested functions: outer -> inner -> undefined
		vm.functions["inner"] = &FunctionDef{
			Name:       "inner",
			Parameters: []FunctionParam{},
			Body: []opcode.OpCode{
				{
		...
		vm.functions["outer"] = &FunctionDef{
			Name:       "outer",
			Parameters: []FunctionParam{},
			Body: []opcode.OpCode{
				{
		...
		_, err := vm.executeCall(opcode)
		if err == nil {
			t.Fatal("expected error to propagate from nested function, got nil")
		}

		// Check that the fatal error propagated through all levels
		var runtimeErr *RuntimeError
		if !errors.As(err, &runtimeErr) {
			t.Fatalf("expected RuntimeError, got %T", err)
		}
		if !runtimeErr.IsFatal() {
			t.Error("expected fatal error to propagate through nested calls")
		}
	})
}
