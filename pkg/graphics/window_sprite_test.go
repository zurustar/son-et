...
func TestWindowSpriteAddChild(t *testing.T) {
	...
	// 子スプライトの絶対位置を確認
	// 親の位置(100, 50) + 子の相対位置(10, 20) + childOffset(4, 24) = (114, 94)
	// childOffset = (borderThickness, borderThickness + titleBarHeight) = (4, 24)
	// これはコンテンツ領域内での正しい位置
	absX, absY := child.AbsolutePosition()
	expectedX := 100.0 + 10.0 + 4.0 // parent.X + child.X + borderThickness
	expectedY := 50.0 + 20.0 + 24.0 // parent.Y + child.Y + borderThickness + titleBarHeight
	if absX != expectedX || absY != expectedY {
		t.Errorf("Expected absolute position (%v, %v), got (%v, %v)", expectedX, expectedY, absX, absY)
	}
...
// TestWindowSpriteGetPicOffset はWindowSprite.GetPicOffsetメソッドをテストする
func TestWindowSpriteGetPicOffset(t *testing.T) {
	sm := NewSpriteManager()
	wsm := NewWindowSpriteManager(sm)
