...
func (tr *TextRenderer) SetFont(name string, size int, opts ...FontOption) error {
	...
	// 極端に大きいサイズは制限（レガシースクリプト対応）
	if size > 200 {
		tr.log.Debug("SetFont: Large size detected, using default",
			"requestedSize", size,
			"defaultSize", 13)
		size = 13
	}
	...
	// フォントを読み込む
	face, err := tr.loadFont(name, size)
	if err != nil {
...
// 要件 5.5: BackMode(mode)が呼ばれたとき、背景モードを設定する（0=透明, 1=不透明）
...
func (tr *TextRenderer) TextWrite(pic *Picture, x, y int, text string) error {
	...
	layer := CreateTextLayer(
		...
		x, y,
		tr.font.Size,
		tr.settings.TextColor,
		pic.ID, // ピクチャーIDを設定
	)

	if layer == nil {
		tr.log.Warn("Failed to create text layer",
			"text", text,
			"x", x,
			"y", y)
		return nil
	...
	draw.Draw(rgba, destRect, layer.Image, image.Point{}, draw.Over)
...
// 埋め込みフォントのサポート
// 注意: 実際のフォントファイルを埋め込む場合は、以下のようにgo:embedディレクティブを使用します。
// 現在はシステムフォントを優先し、見つからない場合はbasicfontにフォールバックします。
//
// 埋め込みフォントを追加する場合:
// 1. pkg/graphics/fonts/ ディレクトリを作成
// 2. NotoSansJP-Regular.ttf などのフォントファイルを配置
// 3. 以下のコメントを解除:
//
// //go:embed fonts/NotoSansJP-Regular.ttf
// var embeddedFontData []byte
//
// func (tr *TextRenderer) loadEmbeddedFont(size float64) (font.Face, error) {
//     tt, err := opentype.Parse(embeddedFontData)
//     if err != nil {
...
// loadEmbeddedFont は埋め込みフォントを読み込む（現在は未実装）
// 将来的にNotoSansJP等の日本語フォントを埋め込む予定
func (tr *TextRenderer) loadEmbeddedFont(size float64) (font.Face, error) {
	// 埋め込みフォントが利用可能になったら実装
	// 現在はbasicfontにフォールバック
	return nil, fmt.Errorf("embedded font not available")
}
