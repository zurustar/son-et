...
// TestCapTitleAllAppliedToNewWindows tests that CapTitleAll sets the default caption
// for windows created after the call
// Validates: Requirements 23.1 - CapTitle(title)は全ての仮想ウィンドウ（まだオープンしていないものも含め）のタイトルを設定する
func TestCapTitleAllAppliedToNewWindows(t *testing.T) {
	wm := NewWindowManager()

	// Set default caption before creating any windows
	wm.CapTitleAll("Default Title")

	// Create a new window (should have the default caption)
	winID, err := wm.OpenWin(1)
	if err != nil {
		t.Fatalf("OpenWin failed: %v", err)
	}

	// Verify the new window has the default caption
	win, err := wm.GetWin(winID)
	if err != nil {
		t.Fatalf("GetWin failed: %v", err)
	}
	if win.Caption != "Default Title" {
		t.Errorf("Expected caption 'Default Title', got '%s'", win.Caption)
	}

	// Create another window
	winID2, err := wm.OpenWin(2)
	if err != nil {
		t.Fatalf("OpenWin failed: %v", err)
	}

	// Verify the second window also has the default caption
	win2, err := wm.GetWin(winID2)
	if err != nil {
		t.Fatalf("GetWin failed: %v", err)
	}
	if win2.Caption != "Default Title" {
		t.Errorf("Expected caption 'Default Title', got '%s'", win2.Caption)
	}

	// Change the default caption
	wm.CapTitleAll("New Default")

	// Create a third window
	winID3, err := wm.OpenWin(3)
	if err != nil {
		t.Fatalf("OpenWin failed: %v", err)
	}

	// Verify the third window has the new default caption
	win3, err := wm.GetWin(winID3)
	if err != nil {
		t.Fatalf("GetWin failed: %v", err)
	}
	if win3.Caption != "New Default" {
		t.Errorf("Expected caption 'New Default', got '%s'", win3.Caption)
	}

	// Verify existing windows were also updated
	win, _ = wm.GetWin(winID)
	if win.Caption != "New Default" {
		t.Errorf("Expected existing window caption 'New Default', got '%s'", win.Caption)
	}
}
