...
// 要件 2.2: 操作順序でLocal_Z_Orderを割り当てる
...
func (ssm *ShapeSpriteManager) CreateLineSpriteWithParent(
	...
	if ss != nil && parent != nil {
		ss.SetParent(parent)
		parent.AddChild(ss.sprite)

		// 要件 2.2, 2.6: 操作順序でLocal_Z_Orderを割り当てる
		// 要件 1.4: 親のZ_Pathを継承し、自身のLocal_Z_Orderを追加する
		if parent.GetZPath() != nil {
			localZOrder := ssm.spriteManager.GetZOrderCounter().GetNext(parent.ID())
			zPath := NewZPathFromParent(parent.GetZPath(), localZOrder)
			ss.sprite.SetZPath(zPath)
			ssm.spriteManager.MarkNeedSort()
		}
	}
	return ss
}
...
func (ssm *ShapeSpriteManager) CreateFillRectSprite(
	...
	// 塗りつぶし矩形を描画
	vector.FillRect(img, 0, 0, float32(width), float32(height), fillColor, false)
...
// GetShapeSpritesInRegion は指定されたピクチャーIDの指定領域内にあるShapeSpriteを取得する
// MovePicで転送元の領域内にあるShapeSpriteを取得するために使用
func (ssm *ShapeSpriteManager) GetShapeSpritesInRegion(picID, srcX, srcY, width, height int) []*ShapeSprite {
	ssm.mu.RLock()
	defer ssm.mu.RUnlock()
	...
	result := make([]*ShapeSprite, 0)
	for _, ss := range sprites {
		// ShapeSpriteの位置が転送領域内にあるかチェック
		// destX, destYはスプライトの描画位置
		if ss.destX >= srcX && ss.destX < srcX+width && ss.destY >= srcY && ss.destY < srcY+height {
...
// CloneShapeSpriteToParent はShapeSpriteをコピーして新しい親スプライトに追加する
// MovePicで転送元のShapeSpriteを転送先にコピーするために使用
// 位置はオフセット（dstX - srcX, dstY - srcY）で調整される
func (ssm *ShapeSpriteManager) CloneShapeSpriteToParent(
	original *ShapeSprite,
	dstPicID int,
	offsetX, offsetY int,
	parent *Sprite,
) *ShapeSprite {
	...
	shapeColor := original.color
	lineSize := original.lineSize
	x1 := original.x1
	y1 := original.y1
	x2 := original.x2
	y2 := original.y2
	...
	switch shapeType {
	case ShapeTypeLine:
		ss = ssm.CreateLineSpriteWithParent(dstPicID, newX1, newY1, newX2, newY2, shapeColor, lineSize, 0, parent)
	case ShapeTypeRect:
		ss = ssm.CreateRectSpriteWithParent(dstPicID, newX1, newY1, newX2, newY2, shapeColor, lineSize, 0, parent)
	case ShapeTypeFillRect:
		ss = ssm.CreateFillRectSpriteWithParent(dstPicID, newX1, newY1, newX2, newY2, shapeColor, 0, parent)
	case ShapeTypeCircle:
		ss = ssm.CreateCircleSpriteWithParent(dstPicID, newX1, newY1, radius, shapeColor, lineSize, 0, parent)
	case ShapeTypeFillCircle:
		ss = ssm.CreateFillCircleSpriteWithParent(dstPicID, newX1, newY1, radius, shapeColor, 0, parent)
	}
