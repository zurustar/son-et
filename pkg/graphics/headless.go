...
// GetWindowCount は開いているウィンドウの数を返す
func (hgs *HeadlessGraphicsSystem) GetWindowCount() int {
	hgs.windowMu.RLock()
	defer hgs.windowMu.RUnlock()
	return len(hgs.windows)
}
...
func (hgs *HeadlessGraphicsSystem) OpenWin(picID int, opts ...any) (int, error) {
	...
	win := &HeadlessWindow{
		...
		PicY:    0,
		BgColor: color.RGBA{0, 0, 0, 255},
		Caption: hgs.defaultCaption, // デフォルトキャプションを適用
		Visible: true,
	...
	if !hasPosition {
		// ピクチャーのサイズを取得してウィンドウサイズを決定
		if pic, ok := hgs.pictures[picID]; ok {
			if win.Width <= 0 || win.Height <= 0 {
				win.Width = pic.Width
				win.Height = pic.Height
			}
		}

		// ウィンドウ装飾を含めた全体サイズ
		totalWidth := win.Width + BorderThickness*2
		totalHeight := win.Height + BorderThickness*2 + TitleBarHeight

		// 中央位置を計算
		win.X = (hgs.virtualWidth - totalWidth) / 2
		win.Y = (hgs.virtualHeight - totalHeight) / 2

		hgs.log.Debug("OpenWin: centering window",
			"winID", id,
			"virtualSize", fmt.Sprintf("%dx%d", hgs.virtualWidth, hgs.virtualHeight),
			"windowSize", fmt.Sprintf("%dx%d", totalWidth, totalHeight),
			"centerPos", fmt.Sprintf("(%d, %d)", win.X, win.Y))
	}
...
// ウィンドウが存在しない場合でもデフォルトキャプションとして保存する
// 新しく作成されるウィンドウにもこのキャプションが適用される
// 受け入れ基準 3.1, 3.2
func (hgs *HeadlessGraphicsSystem) CapTitleAll(title string) {
	hgs.windowMu.Lock()
	defer hgs.windowMu.Unlock()

	// デフォルトキャプションを保存（新しいウィンドウにも適用される）
	hgs.defaultCaption = title

	// 既存の全てのウィンドウのキャプションを設定
