...
// LoadGoTextFace はGoTextFaceを読み込む（オプション）
// システムフォントが利用可能な場合に使用
func (do *DebugOverlay) LoadGoTextFace(fontData []byte, size float64) error {
	do.mu.Lock()
	defer do.mu.Unlock()

	// GoTextFaceSourceを作成（bytes.Readerを使用）
