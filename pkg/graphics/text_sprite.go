...
// TextSprite はテキストレイヤーエントリとスプライトを組み合わせたラッパー構造体
// 要件 5.1〜5.5: テキスト描画をスプライトとして表現する
type TextSprite struct {
	textLayerEntry *TextLayerEntry // 元のテキストレイヤーエントリ情報
...
// CreateTextSpriteFromTextLayerEntry はTextLayerEntryからTextSpriteを作成する
// 既存のTextLayerEntryをスプライトベースに変換するアダプタ
func (tsm *TextSpriteManager) CreateTextSpriteFromTextLayerEntry(entry *TextLayerEntry, zOrder int) *TextSprite {
	if entry == nil {
		return nil
	}
	...
	ts := &TextSprite{
		textLayerEntry: entry,
		sprite:         sprite,
		picID:          entry.GetPicID(),
		text:           entry.GetText(),
		x:              x,
		y:              y,
	}
...
// GetTextLayerEntry は元のTextLayerEntryを返す
func (ts *TextSprite) GetTextLayerEntry() *TextLayerEntry {
	ts.mu.RLock()
	defer ts.mu.RUnlock()
	return ts.textLayerEntry
}
