...
// TestIntegrationOpenWinCentering tests that windows are centered when no position is specified.
// 要件 3.1.1: 位置指定なしの場合はセンタリングする
func TestIntegrationOpenWinCentering(t *testing.T) {
	gs := NewGraphicsSystem("")

	// Create a test picture with known size
	picWidth := 200
	picHeight := 150
	picID, err := gs.CreatePic(picWidth, picHeight)
	if err != nil {
		t.Fatalf("Failed to create picture: %v", err)
	}

	// Open window without position specification (should be centered)
	winID, err := gs.OpenWin(picID)
	if err != nil {
		t.Fatalf("Failed to open window: %v", err)
	}

	// Get window to verify position
	...
	// Window total size includes decorations
	totalWidth := picWidth + BorderThickness*2
	totalHeight := picHeight + BorderThickness*2 + TitleBarHeight
	expectedX := (gs.virtualWidth - totalWidth) / 2
	expectedY := (gs.virtualHeight - totalHeight) / 2

	if win.X != expectedX || win.Y != expectedY {
		t.Errorf("Window not centered: expected (%d, %d), got (%d, %d)",
			expectedX, expectedY, win.X, win.Y)
	}

	t.Logf("✓ Window centered at (%d, %d) for virtual desktop %dx%d",
		win.X, win.Y, gs.virtualWidth, gs.virtualHeight)
}

// TestIntegrationOpenWinWithPositionNotCentered tests that windows with explicit position are not centered.
// 要件 3.2: 位置指定ありの場合は指定位置に配置する
func TestIntegrationOpenWinWithPositionNotCentered(t *testing.T) {
	gs := NewGraphicsSystem("")
	...
	// Open window with explicit position (should NOT be centered)
	explicitX := 50
	explicitY := 100
	winID, err := gs.OpenWin(picID, explicitX, explicitY, 200, 150, 0, 0, 0)
	if err != nil {
		t.Fatalf("Failed to open window: %v", err)
	}
	...
	if win.X != explicitX || win.Y != explicitY {
		t.Errorf("Window position not as specified: expected (%d, %d), got (%d, %d)",
			explicitX, explicitY, win.X, win.Y)
	}

	t.Logf("✓ Window positioned at explicit location (%d, %d)", win.X, win.Y)
}

// TestIntegrationOpenWinCenteringWithZeroPosition tests that explicit (0,0) position is respected.
// 要件 3.2: 明示的に(0, 0)が指定された場合はセンタリングしない
func TestIntegrationOpenWinCenteringWithZeroPosition(t *testing.T) {
	gs := NewGraphicsSystem("")
	...
	// Open window with explicit (0, 0) position (should NOT be centered)
	winID, err := gs.OpenWin(picID, 0, 0, 200, 150, 0, 0, 0)
	if err != nil {
		t.Fatalf("Failed to open window: %v", err)
	}
	...
	// When position is explicitly specified (even as 0,0), it should be respected
	if win.X != 0 || win.Y != 0 {
		t.Errorf("Window position not as specified: expected (0, 0), got (%d, %d)",
			win.X, win.Y)
	}

	t.Logf("✓ Window positioned at explicit (0, 0)")
}
