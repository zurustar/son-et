...
- [ ] 10. ピクチャ描画（MovePic）のスプライト化
  ...
  - [ ] 10.2 MovePic()でスプライトを作成するように変更する
  - [ ] 10.3 drawDrawingEntryOnScreen()をスプライトベースに変更する
  - [ ] 10.4 ピクチャ描画の単体テストを作成する
  - [ ] 10.5 サンプルタイトル（kuma2）でピクチャ描画を確認する

- [ ] 11. キャスト描画のスプライト化
  - [ ] 11.1 CastSpriteラッパー構造体を作成する（Cast + Sprite）
  - [ ] 11.2 PutCast()でCastSpriteを作成するように変更する
  - [ ] 11.3 MoveCast()でCastSpriteを更新するように変更する
  - [ ] 11.4 DelCast()でCastSpriteを削除するように変更する
  - [ ] 11.5 drawCastLayerOnScreen()をスプライトベースに変更する
  - [ ] 11.6 透明色処理をスプライト描画に統合する
  - [ ] 11.7 キャスト描画の単体テストを作成する
  - [ ] 11.8 サンプルタイトル（home）でキャスト描画を確認する

- [ ] 12. テキスト描画のスプライト化
  - [ ] 12.1 TextSpriteラッパー構造体を作成する（TextLayerEntry + Sprite）
  - [ ] 12.2 TextWrite()でTextSpriteを作成するように変更する
  - [ ] 12.3 drawTextLayerOnScreen()をスプライトベースに変更する
  - [ ] 12.4 テキスト描画の単体テストを作成する
  - [ ] 12.5 サンプルタイトル（kuma2）でテキスト描画を確認する

- [ ] 13. 図形描画のスプライト化
  - [ ] 13.1 ShapeSpriteラッパー構造体を作成する（図形情報 + Sprite）
  - [ ] 13.2 DrawLine/DrawRect/DrawCircle/FillRectでShapeSpriteを作成するように変更する
  - [ ] 13.3 図形描画の単体テストを作成する
  - [ ] 13.4 サンプルタイトル（ftile400）で図形描画を確認する

- [ ] 14. 描画システムの統合
  - [ ] 14.1 GraphicsSystem.Draw()をSpriteManager.Draw()ベースに変更する
  - [ ] 14.2 ウインドウ内のスプライトをウインドウの子スプライトとして管理する
  - [ ] 14.3 Z順序の統一（ウインドウ間、ウインドウ内）を実装する
  - [ ] 14.4 統合テストを作成する
  - [ ] 14.5 サンプルタイトル（kuma2, home）で全体動作を確認する

- [ ] 15. 旧レイヤーシステムの削除
  - [ ] 15.1 LayerManagerの使用箇所を確認し、スプライトシステムに置き換える
  - [ ] 15.2 WindowLayerSetの使用箇所を確認し、スプライトシステムに置き換える
  - [ ] 15.3 PictureLayerSetの使用箇所を確認し、スプライトシステムに置き換える
  - [ ] 15.4 CastLayer, DrawingEntry, TextLayerEntryの使用箇所を確認する
  - [ ] 15.5 不要になったレイヤー関連ファイルを削除する
  - [ ] 15.6 不要になったテストファイルを削除する
  - [ ] 15.7 全テストがパスすることを確認する
