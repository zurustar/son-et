...
**2引数形式** - ソースピクチャー全体を転送:
```filly
MovePic(src_pic, dst_pic)
```

- `src_pic`: 移動元ピクチャー番号
- `dst_pic`: 移動先ピクチャー番号

**6引数形式** - ソース領域を指定して転送（dst_x=0, dst_y=0, mode=0）:
```filly
MovePic(src_pic, src_x, src_y, width, height, dst_pic)
```
...
**8引数形式** - ソース領域と転送先位置を指定（mode=0）:
```filly
MovePic(src_pic, src_x, src_y, width, height, dst_pic, dst_x, dst_y)
```
...
- `mode`: 転送モード
  - `0`: 通常
  - `1`: 透明色転送モード
  - `2`〜`9`: シーンチェンジモード（詳細は [描画システム - シーンチェンジ](graphics-system.md#5-シーンチェンジの各モード) を参照）
...
- 9引数形式の全引数に加えて:
- `speed`: シーンチェンジの速度（0=デフォルト、値が大きいほど速い）
...
**4引数形式** - ピクチャーと位置を変更:
```filly
MoveCast(cast_no, pic_no, x, y)
```

- `cast_no`: キャスト番号
- `pic_no`: 新しいソースピクチャー番号
- `x`: 配置位置 X座標
- `y`: 配置位置 Y座標
...
**9引数形式** - 全パラメータを変更:
```filly
MoveCast(cast_no, pic_no, x, y, trans_color, width, height, src_x, src_y)
```
...
```filly
SetFont(size, font_name, charset, avg_width, escapement, orientation, weight, italic, underline, strikeout)
```

**引数**:
- `size`: フォントサイズ
- `font_name`: フォント名
- `charset`: 文字セット (0=英語, 128=日本語, 2=シンボル)
- `avg_width`: 平均の幅 (0=デフォルト)
- `escapement`: 傾斜角度 (0=デフォルト)
- `orientation`: 回転角度 (0=デフォルト)
...
**引数**:
- `filename`: ファイル名（文字列）。相対パスの場合、タイトルディレクトリからの相対パスとして解決される
- `mode`: モードフラグ（整数、省略可）

**引数1つ（filenameのみ）**: ファイルを読み込み専用モードで開く

**引数2つ（filename, mode）**: modeフラグに従ってファイルを開く
...
新規作成フラグ:
- `0x1000`: ファイルが存在しなければ新規作成し、存在すれば内容を切り詰めて開く

フラグはOR結合で組み合わせ可能（例: `0x1001` = 書き専用 + 新規作成）

**戻り値**: 整数ハンドル（1以上の正の整数）

**エラー**: ファイルが存在せず新規作成フラグもない場合はエラー。引数不足の場合もエラー。
...
**引数**:
- `handle`: OpenFで取得した整数ハンドル

**エラー**: 無効なハンドルの場合はエラー。引数不足の場合もエラー。
...
**引数**:
- `handle`: OpenFで取得した整数ハンドル
- `size`: 読み込むバイト数（1, 2, 4のいずれか）

**動作**: 指定されたバイト数をリトルエンディアンの整数値として読み込む

- `size=1`: 1バイトを読み込み、0〜255の整数値として返す
- `size=2`: 2バイトを読み込み、リトルエンディアンの16ビット整数値として返す
- `size=4`: 4バイトを読み込み、リトルエンディアンの32ビット整数値として返す

**戻り値**: 読み込んだ整数値

**エラー**: sizeが1, 2, 4以外の場合はエラー。無効なハンドル、引数不足の場合もエラー。
...
```filly
WriteF(handle, value)
WriteF(handle, value, length)
```

**引数**:
- `handle`: OpenFで取得した整数ハンドル
- `value`: 書き込む整数値
- `length`: 書き込むバイト数（1, 2, 4のいずれか。省略可）

**2引数版（handle, value）**: valueを1バイトとして書き込む

**3引数版（handle, value, length）**: valueをlengthバイトのリトルエンディアンで書き込む

- `length=1`: valueの下位1バイトを書き込む
- `length=2`: valueを2バイトのリトルエンディアンで書き込む
- `length=4`: valueを4バイトのリトルエンディアンで書き込む

**エラー**: lengthが1, 2, 4以外の場合はエラー。無効なハンドル、引数不足の場合もエラー。

#### SeekF
ファイルポインタの移動

```filly
pos = SeekF(handle, offset, origin)
```
...
**戻り値**: 移動後の新しいファイルポインタ位置

**エラー**: 無効なハンドル、引数不足の場合はエラー。
...
**動作**:
- 改行区切りで1行を読み込む（CR, LF, CRLFのいずれにも対応）
- ファイルはShift-JISエンコーディングとして扱い、読み込んだ文字列をUTF-8に変換して返す
- 返す文字列に改行文字は含まない

**戻り値**: 読み込んだ文字列。EOF（ファイル終端）に達した場合は空文字列 `""` を返す
...
**動作**:
- 文字列をUTF-8からShift-JISに変換してファイルに書き込む
- 改行は付加しない（改行が必要な場合はスクリプト側で明示的に書き込む）
...
**フォールスルーなし**: C言語と異なり、FILLYのswitch文はフォールスルーしません。マッチしたcaseの本体のみが実行され、後続のcaseには処理が流れません。break文はcase本体の途中で処理を中断する目的で使用できますが、break文がなくても次のcaseには進みません。

**ループ内でのbreak**: switch文内のbreak文はswitch文のみを終了し、外側のループには影響しません。
...
- [アーキテクチャ](architecture.md) - son-et のアーキテクチャ設計
- [コンパイラ](compiler.md) - コンパイラパイプラインとOpCode仕様
- [実行モデル](execution-model.md) - VM実行モデルとイベントシステム
