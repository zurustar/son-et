...
確定した新規バグは2件（いずれも pkg/vm、**未修正**）:

| # | severity | 場所 | 概要 |
|---|----------|------|------|
| P2-a | 🟠 中〜高（セキュリティ） | [vm.go resolveFilePath](../pkg/vm/vm.go#L1519-L1534) | 無サニタイズのパストラバーサル。OpenF/WriteIni*/GetIni*/MIDI・WAV再生/LoadPic が任意パス（絶対パス・`..`）を読み書き可能。信頼できないタイトル実行時のサンドボックス脱出。**resolveFilePath 1か所の修正で全 sink を塞げる** |
| P2-b | 🟡 中 | [builtins_fileio.go ReadF/StrReadF](../pkg/vm/builtins_fileio.go#L208-L368) | ReadF(直接file) と StrReadF(bufio) のバッファ非同期。同一ハンドルで混在させると ReadF が誤位置を読む |
