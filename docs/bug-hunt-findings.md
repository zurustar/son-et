...
# フェーズ2: 未走査領域の調査（2026-06-30〜）

フェーズ1（タスク1〜9）で未走査だった builtins / 描画詳細 / window・title・script を調査する。
※このフェーズの所見は未修正（記録のみ）。

## P2-1. VM builtins_fileio (pkg/vm/builtins_fileio.go) — 完了

### 検出した問題

1. **【確定・中〜高severity / セキュリティ】ファイルパスが無サニタイズ＝パストラバーサル/任意ファイルアクセス**
   - 場所: [vm.go:1519-1534 `resolveFilePath`](../pkg/vm/vm.go#L1519-L1534) と [builtins_fileio.go:52-105 `OpenF`](../pkg/vm/builtins_fileio.go#L52-L105)
   - `resolveFilePath` は絶対パスをそのまま返し、相対パスは `filepath.Join(titlePath, filename)` するだけ。`..` を含む相対パスは Join がクリーンしても**タイトルディレクトリ外へ脱出できる**（例: `OpenF("../../../../etc/passwd")`）。絶対パス（例: `OpenF("/etc/passwd", 0x1001)`）も素通し。
   - `OpenF` は `FileCreateNew(0x1000)` で `os.O_CREATE|os.O_TRUNC` 書き込みも可能 → **信頼できないタイトルが任意の場所のファイルを読み取り・上書きできる**。同じ `resolveFilePath` は MIDI/WAV 再生や画像読み込みでも使われる。
   - 影響度は脅威モデル次第（タイトルが信頼できるなら低）。だが本アプリは外部タイトル読み込み・タイトル選択機能を持つため、未知タイトルを動かす場合はサンドボックス脱出になる。
   - 修正案: `filepath.Clean` 後に `titlePath` 配下に収まるか検証（`strings.HasPrefix(cleaned, titleAbs)` 等）し、絶対パス・`..` 脱出を拒否する。

2. **【確定・中severity】`ReadF`(直接 file 読み) と `StrReadF`(bufio.Reader 読み) のバッファ非同期**
   - 場所: [builtins_fileio.go:208-256 `ReadF`](../pkg/vm/builtins_fileio.go#L208-L256) は `entry.file` から直接 `io.ReadFull`。[StrReadF:326-368](../pkg/vm/builtins_fileio.go#L326-L368) は `entry.reader`(bufio) を遅延生成して読む。
   - 同一ハンドルで StrReadF→ReadF と混在させると、bufio が先読みバッファした分を ReadF がスキップしてしまい、**ReadF が誤った位置のデータを読む**。`SeekF` は `ResetReader` でバッファを捨てるので seek を挟めば回避できるが、混在のまま使うと壊れる。
   - 修正案: 読み取りを常に bufio 経由に統一する（ReadF も entry.reader から読む）か、StrReadF 後の生 read を禁止/警告する。

### 確認できた“非バグ”（参考）
- OpenF の mode フラグ解釈（アクセス属性マスク・新規作成）、SeekF の origin マッピング、ReadF/WriteF の size 検証、CR/LF/CRLF 行区切り処理、Shift-JIS⇔UTF-8 変換は妥当。
- ファイルハンドルは fileHandleTable で管理され、VM 終了時 `CloseAll()` される（フェーズ1 app 調査済み）。

## P2-2. VM builtins_system (pkg/vm/builtins_system.go) — 完了

### 検出した問題

1. **【確定・P2-1と同根】INIファイル操作も無サニタイズのパストラバーサル**
   - 場所: [builtins_system.go GetIniStr/GetIniInt/WriteIniStr/WriteIniInt](../pkg/vm/builtins_system.go#L245-L540)（いずれも `v.resolveFilePath(filename)` 経由、例: L267/L330/L390/L474）
   - `WriteIniStr`/`WriteIniInt` は任意パスにファイルを書き込める。P2-1 #1（resolveFilePath の無検証）の追加の sink。パストラバーサル修正は resolveFilePath 側で一括対処すれば OpenF・Ini・音声・画像すべてに効く。

### 確認できた“非バグ”（参考）
- **`Shell` / `MCI` / `StrMCI` はスタブ（no-op）**で未実装（[L555-572](../pkg/vm/builtins_system.go#L555-L572)）。ログを出して 0/"" を返すのみ。**任意コマンド実行(RCE)はできない**。
- `PostMes` の messageType→EventType マッピングは switch+default で範囲外も USER 扱い、安全（[L580-630](../pkg/vm/builtins_system.go#L580-L630)）。
- del_me/del_us/del_all、ExitTitle、GetSysTime、MsgBox 等は素直な実装。

### 影響範囲メモ（パストラバーサル）
`resolveFilePath` を経由する全 sink: `OpenF`（読み書き/新規作成）, `GetIniStr/GetIniInt/WriteIniStr/WriteIniInt`, MIDI/WAV 再生, 画像読み込み。**1か所（resolveFilePath）の修正で全経路を塞げる**。

## P2-3. builtins_graphics / script / title / window / headless — 完了（確定バグなし）

以下は精査したが確定バグなし。参考として記録。

- **builtins_graphics.go (pkg/vm, 1062行)**: LoadPic/CreatePic/MovePic/DrawRect/… は型チェック付き（toInt64 等）の薄いラッパーで、実処理は GraphicsSystem（ebiten 委譲、境界安全）へ委譲。未チェックの型アサーションなし。`LoadPic` はファイル名を `resolveFilePath` に渡すため **P2-1 のパストラバーサル経路を共有**（画像読み込みsink）。
- **pkg/script/script.go (128行)**: .TFY 走査＋Shift-JIS→UTF-8 変換。素直で問題なし（UTF-8ファイルを Shift-JIS デコードすると化ける設計上の前提は既知）。
- **pkg/title/title.go (365行)**: #info メタデータ抽出（lexer 使用）と title.json 読み込み（JSON パースエラーは空扱い）。境界・nil 処理とも妥当。
- **pkg/window/window.go (657行)**:
  - `screenToVirtual`（[L437-484](../pkg/window/window.go#L437-L484)）: スケール除算は float なのでゼロ除算でも +Inf となり panic しない。範囲クランプあり。
  - `updateSelection`（[L205-255](../pkg/window/window.go#L205-L255)）: `selectedIndex` は上下キーで [0, len-1] に制限され、選択モードは titles>=2 前提のため `&g.titles[idx]` は安全。
- **pkg/graphics/headless.go (964行)**: リソース記録＋ログ出力のみで**実ピクセル描画をしない**（`.Pix`/`SetPixels` 等なし）。maxPictures/maxWindows のリソース上限チェックあり。フェーズ1で「要精査」としたが安全と確認。
...
| # | severity | 状態 | 場所 | 概要 |
|---|----------|------|------|------|
| P2-a | 🟠 中〜高（セキュリティ） | ✅修正 | [vm.go resolveFilePath](../pkg/vm/vm.go) / [fileutil RealFS](../pkg/fileutil/fs.go) | 無サニタイズのパストラバーサル。OpenF/WriteIni*/GetIni*/MIDI・WAV再生（vm経路）と画像読み込み（graphics→RealFS経路）が任意パスを読み書き可能だった |
| P2-b | 🟡 中 | ✅修正 | [builtins_fileio.go ReadF/StrReadF](../pkg/vm/builtins_fileio.go) / [file_handle_table.go](../pkg/vm/file_handle_table.go) | ReadF(直接file) と StrReadF(bufio) のバッファ非同期。同一ハンドルで混在させると ReadF が誤位置を読む |

Shell/MCI/StrMCI は未実装スタブで RCE なし。描画系・ローダ・ウィンドウは概ね健全。

### P2-a 修正内容（2026-07-01）
- **VM経路**: `resolveFilePath` を `(string, error)` に変更し、titlePath 配下に封じ込め（絶対パス拒否・`..` 脱出拒否）。全呼び出し元（OpenF / GetIniStr / GetIniInt / WriteIniStr / WriteIniInt / PlayMIDI / PlayWAVE）でエラー伝播。回帰テスト `pkg/vm/resolve_filepath_test.go` 追加。
- **graphics経路**: 画像読み込みは `vm.resolveFilePath` を通らず `fileutil.RealFS`（basePath付）経由だった（フェーズ2で「resolveFilePath 経由」と記したのは誤り）。`RealFS.resolvePath` も `..` を root 吸収で無害化（`filepath.Clean(sep+name)`）。read-only だが同種の脆弱性を閉塞。
- titlePath 未設定時（テスト等）は従来通り filename 素通し（後方互換）。EmbedFS はバイナリ埋め込み＝信頼済みのため対象外。
- 全パッケージ build 成功、pkg/vm・fileutil・graphics テスト通過。

### P2-b 修正内容（2026-07-01）
- 読み取りのバッファリングを廃止し、`ReadF`（バイナリ）も `StrReadF`（行）も `*os.File` を直接読むよう統一。これで両者の論理位置＝ファイル位置となり、混在しても読み飛ばしが起きない。
- `fileEntry` から `reader *bufio.Reader` フィールドと `FileHandleTable.ResetReader` を削除。`SeekF` の `ResetReader` 呼び出しも削除（バッファが無いので不要）。
- 行読み取りは `readLineFromFile(*os.File)` に置換。CR 単独時は次バイトを読んで LF でなければ 1 バイト seek で押し戻す（CR/LF/CRLF 対応）。
- 副次効果: 先読みでファイル位置が進む問題が無くなり、読み取り後の `SeekF(SEEK_CUR)` も正確になった。
- 回帰テスト `pkg/vm/fileio_mixed_read_test.go` 追加（ReadF→StrReadF→ReadF の混在で位置がずれないことを検証）。旧 `TestFileHandleTable_ResetReader` は削除。
- pkg/vm 全テスト通過。

---

# フェーズ1 軽微所見の対応（2026-07-02）

フェーズ1で「軽微」として記録した所見のうち、価値が高く低リスクなものを修正した（ブランチ fix/phase1-minor）。各修正に回帰テストを追加、全パッケージのテスト通過。

| 所見 | 対応 |
|------|------|
| Lexer: `0x`（桁なし）が INT トークンになる | [lexer.go readHexNumber](../pkg/compiler/lexer/lexer.go) で桁数0なら `TOKEN_ILLEGAL` を返すよう変更。lexer 段で検出。 |
| Lexer: 未終端の文字列/コメントが黙って受理される | Lexer に `errors []*LexerError` を追加し、`readString`/`skipMultiLineComment` の EOF 到達時に記録。`Errors()` と `TokenizeWithErrors` で取得可能に（トークン出力は不変）。 |
| Parser: lexer のエラーを捨てている | `New()` で `l.Errors()`（未終端エラー）を parser エラーへ surface。ILLEGAL は従来通りパース時 `noPrefixParseFnError` で報告されるため二重計上を避け、未終端系のみ追加。 |
| VM: `Scope.Set` の unlock→再lock がデータ競合の窓 | 親スコープ探索中も `s.mu` を保持するよう変更（ロック獲得は常に child→parent で一貫しデッドロックしない）。競合窓を除去。 |
| VM: イベントキューのソートが非安定 | `sort.Slice` → `sort.SliceStable`（[event.go Push](../pkg/vm/event.go)）。同一タイムスタンプでも登録/到着順（FIFO）を保持。 |
| VM: 大文字小文字無視の関数探索が map 線形走査（非決定的・O(n)） | `builtinsLower`/`functionsLower` の小文字インデックスを登録時に構築し、`executeCall` の探索を O(1)・決定的に（[executor.go](../pkg/vm/executor.go)）。 |

追加/変更テスト: `pkg/compiler/lexer/lexer_errors_test.go`（新規）、既存 lexer/parser/vm テスト通過。

### 未対応（意図的に見送り）
- **Compiler**: 配列サイズ drop / `return`=Call / `for(;;)` nil条件 → VM 側で吸収され挙動上無害（フェーズ1で確認済み）。
- **cli**: `reorderArgs` の bool フラグ ハードコード列挙 → 現状の定義済みフラグでは破綻しないため据え置き。
- **StrCode の2バイト生成 / Shift-JIS 前提** → 設計上の前提として据え置き。
