...
// Global Conductor Ticker (LEGACY - kept for backward compatibility)
// The conductor is no longer used for tick generation.
// Ticks are now generated by MidiStream.Read() using wall-clock time.
var (
	// Audio Sync Globals (LEGACY - not used in production)
	// These were used by the old sample-based tick calculation system.
	// Kept for backward compatibility but not actively used.
	globalTempoMap []TempoEvent
	globalPPQ      int   = 480
	currentSamples int64 // UNUSED: Legacy sample counter, replaced by wall-clock time
)

// StartConductor sets up the sync engine (LEGACY).
//
// **LEGACY FUNCTION - Kept for backward compatibility**
//
// This function was part of the old tick generation system that used
// a separate goroutine to generate ticks. It is no longer used in production.
//
// Ticks are now generated by MidiStream.Read() using wall-clock time,
// which is more accurate and doesn't require a separate goroutine.
//
// The function still sets up global variables for backward compatibility,
// but these are not actively used by the new timing system.
func StartConductor(tempoMap []TempoEvent, ppq int) {
	// Setup globals for backward compatibility
	globalTempoMap = tempoMap
	globalPPQ = ppq
	currentSamples = 0 // UNUSED: Legacy sample counter

	// Update Engine Global
	GlobalPPQ = ppq

	// NOTE: We don't start a goroutine anymore.
	// Ticks are driven by MidiStream.Read() using wall-clock time
}

// StartConductorStub is a legacy placeholder (UNUSED).
//
// **LEGACY FUNCTION - Not used**
//
// This function was a placeholder during the transition from the old
// conductor-based tick generation to the new wall-clock time based system.
// It is kept for backward compatibility but serves no purpose.
			...
			for curr < trackEnd {
				...
				if status == 0xFF { // Meta Event
					...
					if typeByte == 0x51 && lenByte == 3 {
						...
						// Valid range: 20 BPM (3,000,000 us/beat) to 300 BPM (200,000 us/beat)
						// Invalid: zero, negative, or extremely large/small values
						if usPerBeat <= 0 || usPerBeat < 200000 || usPerBeat > 3000000 {
							fmt.Printf("Warning: Invalid tempo value %d at tick %d, using default 120 BPM\n", usPerBeat, currentTick)
							usPerBeat = 500000 // Default 120 BPM
						}
...
//
// MidiStream is responsible for:
// 1. Rendering audio samples from the MIDI synthesizer
// 2. Calculating the current MIDI tick from wall-clock time
// 3. Delivering tick notifications to the VM for script synchronization
// 4. Detecting MIDI playback end and triggering MIDI_END event
//
// # Wall-Clock Time Based Tick Calculation
//
// The Read() method uses wall-clock time (time.Since(startTime)) as the
// primary timing source instead of audio sample counts. This provides:
//
...
//
// To prevent animation frame skipping, Read() delivers ALL ticks from
// lastDeliveredTick+1 to currentTick sequentially. This ensures that
// even if processing is delayed, no ticks are skipped.
//
// Example: If lastDeliveredTick=100 and currentTick=105, Read() will call:
//
//	NotifyTick(101), NotifyTick(102), NotifyTick(103), NotifyTick(104), NotifyTick(105)
//
// # MIDI End Detection
//
// Read() compares currentTick >= totalTicks to detect when MIDI playback
// has finished. When detected, it:
// 1. Sets midiFinished global flag to true
// 2. Triggers MIDI_END event if handler is registered
// 3. Stops sending tick notifications
//
// This allows the VM to handle pending Wait operations gracefully.
type MidiStream struct {
	sequencer     *meltysynth.MidiFileSequencer
	leftBuf       []float32
	rightBuf      []float32
	tickGenerator *TickGenerator // Calculates ticks from wall-clock time
	startTime     time.Time      // Wall-clock time when playback started
	totalTicks    int64          // Total ticks in MIDI file (for end detection)
	endReported   bool           // Flag to ensure MIDI_END is triggered only once
	readCallCount int            // Counter for debug logging
}

// Read implements io.Reader for audio playback and MIDI tick synchronization.
//
// This method is called by the Ebiten audio system from the audio thread
// to fill audio buffers. It performs three critical functions:
//
// 1. **Audio Rendering**: Renders audio samples from the MIDI synthesizer
// 2. **Tick Calculation**: Calculates current MIDI tick from wall-clock time
// 3. **Tick Delivery**: Sends tick notifications to the VM for script synchronization
//
// # Wall-Clock Time Based Timing
//
// The method uses time.Since(startTime) to get elapsed time in seconds,
// then calls tickGenerator.CalculateTickFromTime(elapsed) to get the current tick.
// This approach eliminates cumulative drift from audio buffer processing delays.
//
// # Sequential Tick Delivery
//
// To prevent animation frame skipping, all ticks from lastDeliveredTick+1 to
// currentTick are delivered sequentially via NotifyTick(). This ensures that
// even if processing is delayed, no ticks are skipped.
//
// # MIDI End Detection
//
// The method checks if currentTick >= totalTicks to detect MIDI playback end.
// When detected, it sets midiFinished=true and triggers MIDI_END event.
//
// # Thread Safety
//
// This method runs in the audio thread. It avoids locks to prevent audio glitches.
// Tick notifications are sent to the main thread via NotifyTick() which handles
// cross-thread communication safely.
func (s *MidiStream) Read(p []byte) (n int, err error) {
	...
	if s.tickGenerator != nil {
		...
		// PRIMARY TIMING SOURCE: Wall-clock time since playback started
		// This is more accurate than counting audio samples because it's not
		// affected by audio buffer processing delays or variable buffer sizes
		elapsed := time.Since(s.startTime).Seconds()

		// Calculate current tick from elapsed time using tempo map
		// This properly accounts for tempo changes throughout the MIDI file
		// Formula: ticks = elapsed_time * (tempo_bpm / 60) * ppq
		currentTick := s.tickGenerator.CalculateTickFromTime(elapsed)

		// DEBUG: Log tick comparison every 1000 Read() calls (approximately every 5-10 seconds)
		if debugLevel >= 2 && s.readCallCount%1000 == 0 {
			fmt.Printf("[%s] MidiStream.Read: call#%d, currentTick=%d, totalTicks=%d, midiFinished=%v, endReported=%v, elapsed=%.3fs\n",
				time.Now().Format("15:04:05.000"), s.readCallCount, currentTick, s.totalTicks, midiFinished, s.endReported, elapsed)
		}

		// Check if we've reached the end of the MIDI file
		// This is the primary MIDI end detection mechanism
		if int64(currentTick) >= s.totalTicks && !s.endReported {
			fmt.Printf("[%s] MidiStream.Read: MIDI playback ended (currentTick=%d >= totalTicks=%d), triggering MIDI_END event\n",
				time.Now().Format("15:04:05.000"), currentTick, s.totalTicks)
			fmt.Printf("[%s] MidiStream.Read: Setting midiFinished=true, endReported=true\n",
				time.Now().Format("15:04:05.000"))
			s.endReported = true
			midiFinished = true // Set global flag so VM can detect MIDI end
		...
		// SEQUENTIAL TICK DELIVERY: Notify all ticks from lastDeliveredTick+1 to currentTick
		// This ensures we don't skip any ticks even if processing is delayed.
		// Example: If lastDeliveredTick=100 and currentTick=105, we deliver ticks 101,102,103,104,105
		// This prevents animation frame skipping and maintains smooth synchronization.
		// But don't exceed totalTicks to avoid delivering ticks past the end of the MIDI file
